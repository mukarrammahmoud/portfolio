...
// @ts-expect-error - imagesloaded has no type definitions
...
export function StaggeredGrid({
  ...
  useEffect(() => {
    ...
    if (gridFullRef.current) {
      ...
      const columns: Array<Array<Element>> = Array.from(
        { length: numColumns },
        () => [],
      )
  ...
  const mixedGridItems: Array<string | 'BENTO_GROUP'> = Array.from(
    { length: 21 },
        ...
        >
          ...
          {mixedGridItems.map((item, i) => {
            ...
            if (typeof item === 'string') {
              const Icon =
                i % 3 === 0 ? FaGithub : i % 3 === 1 ? FaSlack : FaTwitter
              const label =
